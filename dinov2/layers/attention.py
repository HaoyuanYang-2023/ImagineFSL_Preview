...
class AttentionDr(nn.Module):
    def __init__(
        ...
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(qkv_dim, dim, bias=proj_bias)
        self.proj_drop = nn.Dropout(proj_drop)
...
class MemEffAttentionDr(AttentionDr):
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.qkv_dim // self.num_heads)

        q, k, v = unbind(qkv, 2)

        x = memory_efficient_attention(q, k, v, attn_bias=attn_bias)
        x = x.reshape([B, N, self.qkv_dim])
